...
namespace JoostMod.Tiles
{
	internal sealed class JoostGlobalTile : GlobalTile
	{
		public override bool Drop(int i, int j, int type)
		{
			...
			{
				Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 32, Mod.Find<ModItem>("OldNail").Type);
			}
            if (type == TileID.Plants || type == TileID.Plants2 || type == TileID.JunglePlants || type == TileID.JunglePlants2)
            {
                if (WorldGen.genRand.NextBool(2) && Main.player[(int)Player.FindClosest(new Vector2((float)(i * 16), (float)(j * 16)), 16, 16)].HasItem(Mod.Find<ModItem>("MightyBambooShoot").Type))
                {
