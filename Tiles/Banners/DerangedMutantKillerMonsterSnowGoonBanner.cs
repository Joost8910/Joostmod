...
namespace JoostMod.Tiles.Banners     //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
{
    public class DerangedMutantKillerMonsterSnowGoonBanner : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 48, Mod.Find<ModItem>("DerangedMutantKillerMonsterSnowGoonBanner").Type);//this defines what to drop when this tile is destroyed
        }
