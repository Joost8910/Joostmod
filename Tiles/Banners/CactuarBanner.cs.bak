...
{
    public class CactuarBanner : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            name.SetDefault("Cactuar Banner");
            AddMapEntry(new Color(123, 44, 122), name); //this defines the color and the name when you see this tile on the map
        }

        public override void KillMultiTile(int i, int j, int frameX, int frameY)
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 48, ModContent.ItemType<Items.Placeable.CactuarBanner>());//this defines what to drop when this tile is destroyed
        }
        ...
        public override void NearbyEffects(int i, int j, bool closer)   //this make so the banner give an effect to nearby players
        {
            ...
            {
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("Cactuar").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("CrimsonCactuar").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("CorruptCactuar").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("HallowedCactuar").Type] = true;
