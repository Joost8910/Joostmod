...
namespace JoostMod.Tiles.Banners     //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
{
    public class CactoidBanner : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 48, ModContent.ItemType<Items.Placeable.CactoidBanner>());//this defines what to drop when this tile is destroyed
        }
        ...
        public override void NearbyEffects(int i, int j, bool closer)   //this make so the banner give an effect to nearby players
        {
            ...
            {
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("Cactoid").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("CrimsonCactoid").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("CorruptCactoid").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("Cactite").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("CrimsonCactite").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("CorruptCactite").Type] = true;
