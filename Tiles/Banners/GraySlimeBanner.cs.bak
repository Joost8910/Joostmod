...
namespace JoostMod.Tiles.Banners     //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
{
    public class GraySlimeBanner : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 48, ModContent.ItemType<Items.Placeable.GraySlimeBanner>());//this defines what to drop when this tile is destroyed
        }
        ...
        public override void NearbyEffects(int i, int j, bool closer)   //this make so the banner give an effect to nearby players
        {
            ...
            {
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("GraySlime").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("GraySlime2").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("GraySlime3").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("GraySlime4").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("GraySlime5").Type] = true;
                Main.SceneMetrics.NPCBannerBuff[Mod.Find<ModNPC>("GraySlime6").Type] = true;
