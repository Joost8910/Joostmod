...
namespace JoostMod.NPCs.Bosses.SAX
{
    ...
    public class SAXMutant : ModNPC
    {
        ...
        {
            if (NPC.localAI[0] > 90 && NPC.localAI[0] < 120 && NPC.localAI[2] <= 0 && NPC.velocity.Y == 0 && hit.Damage > 1000)
            {
                NPC.localAI[2]++;
                NPC.netUpdate = true;
            }
        ...
        public override void ModifyHitByProjectile(Projectile projectile, ref NPC.HitModifiers modifiers)
        {
            if (NPC.localAI[0] <= 60)
            {
                modifiers.FinalDamage *= NPC.localAI[0] / 60f;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Vector2 drawPosition = new Vector2(NPC.position.X - Main.screenPosition.X + NPC.width / 2 - texture.Width / xFrameCount / 2f + vector.X, NPC.position.Y - Main.screenPosition.Y + NPC.height - texture.Height / Main.npcFrameCount[NPC.type] + yOffset + vector.Y);
            if ((NPC.ai[1] >= 100 || NPC.ai[2] >= 40) && NPC.localAI[3] < 100)
            {
                for (int i = 0; i < NPC.oldPos.Length; i++)
                {
                    Color color2 = color * ((NPC.oldPos.Length - i) / (float)NPC.oldPos.Length) * 0.5f;
                    Vector2 drawPos = new Vector2(NPC.oldPos[i].X - Main.screenPosition.X + NPC.width / 2 - texture.Width / xFrameCount / 2f + vector.X, NPC.oldPos[i].Y - Main.screenPosition.Y + NPC.height - texture.Height / Main.npcFrameCount[NPC.type] + yOffset + vector.Y);
