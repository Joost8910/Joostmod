    ...
    public class WaterElemental : ModNPC
    {
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (NPC.life <= 0)
            {
                for (int k = 0; k < 30; k++)
                {
                    Dust.NewDust(NPC.position, NPC.width, NPC.height, 103, 2.5f * hit.HitDirection, Main.rand.Next(-5, 5), 0, default, 0.7f);
                }
            }
            for (int k = 0; k < 4; k++)
            {
            ...
            if (Main.expertMode)
            {
                ...
                if (NPC.ai[3] >= 2)
                {
                    ...
                    if (NPC.ai[3] < 80)
                    {
                        ...
                        {
                            Dust dust = Dust.NewDustDirect(NPC.Center - new Vector2(10, 10), 20, 20, 172, NPC.velocity.X * 0.8f, NPC.velocity.Y * 0.8f, 0, default, NPC.scale);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Vector2 vect = new Vector2(texture.Width / xFrameCount / 2, texture.Height / Main.npcFrameCount[NPC.type] / 2);
            drawColor *= 0.8f;
            ...
            spriteBatch.Draw(texture, new Vector2(NPC.position.X - Main.screenPosition.X + NPC.width / 2 - texture.Width / xFrameCount / 2f + vect.X, NPC.position.Y - Main.screenPosition.Y + NPC.height - texture.Height / Main.npcFrameCount[NPC.type] + 4f + vect.Y), new Rectangle?(rect), drawColor, NPC.rotation, vect, NPC.scale, SpriteEffects.None, 0f);

            Texture2D tex = (Texture2D)ModContent.Request<Texture2D>($"{Texture}_Hand");
            ...
            spriteBatch.Draw(tex, new Vector2(NPC.position.X - Main.screenPosition.X + NPC.width / 2 - tex.Width / 2f + drawOrigin.X + offSet.X * NPC.scale, NPC.position.Y - Main.screenPosition.Y + NPC.height / 2 + offSet.Y * NPC.scale), new Rectangle?(rect2), drawColor, NPC.rotation, drawOrigin, NPC.scale, SpriteEffects.None, 0f);
            spriteBatch.Draw(tex, new Vector2(NPC.position.X - Main.screenPosition.X + NPC.width / 2 - tex.Width / 2f + drawOrigin.X + offSet2.X * NPC.scale, NPC.position.Y - Main.screenPosition.Y + NPC.height / 2 + offSet2.Y * NPC.scale), new Rectangle?(rect2), drawColor, NPC.rotation, drawOrigin, NPC.scale, SpriteEffects.FlipHorizontally, 0f);
