        ...
        */
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (NPC.life <= 0)
            {
                for (int k = 0; k < 20; k++)
                {
                    Dust.NewDust(NPC.position, NPC.width, NPC.height, 127, 2.5f * hit.HitDirection, Main.rand.Next(-5, 5), 0, default, 0.7f);
                }
            }
        }
        ...
        public override void AI()
        {
            ...
            if (Main.expertMode && NPC.localAI[0] > 800 && NPC.ai[3] < sphereRate && NPC.ai[3] > 60)
            {
                ...
                else
                {
                    ...
                    if (NPC.localAI[1] >= 40)
                    {
                        ...
                        Dust.NewDustDirect(NPC.position, NPC.width, NPC.height, DustID.Torch, -NPC.velocity.X, -NPC.velocity.Y, 0, default, 4f).noGravity = true;
                    }
                    if (NPC.localAI[1] > 90)
                    {
                        NPC.width = 32;
            ...
            else
            {
                ...
                if (NPC.ai[3] > sphereRate)
                {
                    ...
                    {
                        Dust dust = Dust.NewDustDirect(spherePos - new Vector2(5, 5), 10, 10, 6, NPC.velocity.X * 0.8f, NPC.velocity.Y * 0.8f, 0, default, 1.5f);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            spriteBatch.Draw(texture, new Vector2(NPC.position.X - Main.screenPosition.X + NPC.width / 2 - texture.Width / xFrameCount / 2f + vect.X, NPC.position.Y - Main.screenPosition.Y + NPC.height - texture.Height / Main.npcFrameCount[NPC.type] + 4f + vect.Y), new Rectangle?(rect), alpha, NPC.rotation, vect, 1f, effects, 0f);
            return false;
        }
