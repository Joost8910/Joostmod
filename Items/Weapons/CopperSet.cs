...
namespace JoostMod.Items.Weapons
{
    public class CopperSet : ModItem
    {
        ...
        public override void SetDefaults()
        {
            ...
            item.autoReuse = true;
            item.shoot = mod.ProjectileType("CopperHatchet");
            item.shootSpeed = 12f;
            item.crit = 4;
        }
        ...
        public override bool Shoot(Player player, ref Microsoft.Xna.Framework.Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            if (wep == 1)
            {
                Projectile.NewProjectile(position.X, position.Y, (speedX * 2), (speedY * 2), mod.ProjectileType("CopperFlail"), (damage), knockBack, player.whoAmI);
            }
            if (wep == 2)
            {
            ...
            if (wep == 3)
            {
                Projectile.NewProjectile(position.X, position.Y, (speedX), (speedY), 121, (damage), knockBack, player.whoAmI);
            }
            if (wep == 0)
            {
                Projectile.NewProjectile(position.X, position.Y, (speedX), (speedY), mod.ProjectileType("CopperHatchet"), (damage), knockBack, player.whoAmI);
            }
            return false;
        }
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(null, "CopperFlail");
