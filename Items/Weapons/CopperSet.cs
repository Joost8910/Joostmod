...
namespace JoostMod.Items.Weapons
{
    public class CopperSet : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (wep == 1)
            {
                Projectile.NewProjectile(source, position.X, position.Y, (velocity.X * 2), (velocity.Y * 2), Mod.Find<ModProjectile>("CopperFlail").Type, (damage), knockback, player.whoAmI);
            }
            if (wep == 2)
            {
            ...
            if (wep == 0)
            {
                Projectile.NewProjectile(source, position.X, position.Y, (velocity.X), (velocity.Y), Mod.Find<ModProjectile>("CopperHatchet").Type, (damage), knockback, player.whoAmI);
            }
            return false;
        }
