...
namespace JoostMod.Items.Weapons
{
	public class LeadSet : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			item.autoReuse = true;
			item.shoot = mod.ProjectileType("LeadHatchet");
			item.shootSpeed = 12f;
            item.crit = 4;
        }
		...
		public override bool Shoot(Player player, ref Microsoft.Xna.Framework.Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			if (wep == 1)
			{
				Terraria.Projectile.NewProjectile(position.X, position.Y, (speedX * 2), (speedY * 2), mod.ProjectileType("LeadFlail"), (damage), knockBack, player.whoAmI);
			}
			if (wep == 2)
			{
			...
			if (wep == 0)
			{
				Terraria.Projectile.NewProjectile(position.X, position.Y, (speedX), (speedY), mod.ProjectileType("LeadHatchet"), (damage), knockBack, player.whoAmI);
			}
			return false;
		}
			public override void AddRecipes()
		{
			...
			recipe.AddIngredient(null, "LeadFlail");
			recipe.AddIngredient(ItemID.LeadBow);
			recipe.AddIngredient(null, "LeadStaff");
			recipe.AddIngredient(null, "LeadHatchet", 3);
