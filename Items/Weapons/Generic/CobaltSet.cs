...
namespace JoostMod.Items.Weapons.Generic
{
    public class CobaltSet : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (wep == 0)
            {
                Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 12, velocity.Y * 12, ModContent.ProjectileType<Projectiles.Thrown.CobaltChainedchainsaw>(), damage, knockback, player.whoAmI);
            }
            return false;
        }
