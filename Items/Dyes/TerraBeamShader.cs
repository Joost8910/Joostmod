...
namespace JoostMod.Items.Dyes
{
    ...
    public class TerraBeamShader : ModItem
    {
        ...
        public override void SetStaticDefaults()
        {
            if (!Main.dedServ)
            {
                GameShaders.Armor.BindShader(
                    Item.type,
                    new ArmorShaderData(new Ref<Effect>(Mod.Assets.Request<Effect>("Shaders/JuiceProjectileShaders", AssetRequestMode.ImmediateLoad).Value), "GungnirBeamShaderPass").UseColor(new Color(0.71f, 0.90f, 0.11f)).UseSecondaryColor(new Color(0.4f, 0.82f, 1f)) // Be sure to update the effect path and pass name here.
                );
            }
        }
