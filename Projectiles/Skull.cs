		...
		public override void AI()
		{
			...
			for(int i = 0; i < 200; i++)
			{
				...
				if(distance < 480f && !target.friendly && target.active && target.type != 488 && !target.dontTakeDamage)
				{
					...
					{
						Projectile.NewProjectile(Projectile.Center.X, Projectile.Center.Y, shootToX, shootToY, Mod.Find<ModProjectile>("ShadowBolt").Type, Projectile.damage, Projectile.knockBack, Projectile.owner); //Spawning a projectile
