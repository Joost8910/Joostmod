...
{
    public class TrueHallowedFlail : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            player.itemRotation = (float)Math.Atan2((double)(dir.Y * player.direction), (double)(dir.X * player.direction));
            ...
            else if (Projectile.ai[0] == 1f)
            {
                ...
                float num213 = 300f;
                num213 *= 2.5f; // Distance when it forces back
                num211 *= 1.5f; // Floatiness?
                num212 *= 1.5f; // Comeback Speed
                ...
                int num216 = 100;
                if (num209 > num216 || !Projectile.tileCollide)
                {
                    ...
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center.X, Projectile.Center.Y, Projectile.velocity.X, Projectile.velocity.Y, Mod.Find<ModProjectile>("TrueHallowedFlailBeam").Type, (int)(Projectile.damage * 1.0f), 0, Projectile.owner);
        ...
        {
            Texture2D texture = (Texture2D)Mod.Assets.Request<Texture2D>("JoostMod/Projectiles/TrueHallowedFlail_Chain");
            Player player = Main.player[Projectile.owner];
            ...
            while (flag)
            {
                ...
                else
                {
                    ...
                    Main.EntitySpriteDraw(texture, position - Main.screenPosition, sourceRectangle, color2, rotation, origin, Projectile.scale, SpriteEffects.None, 0);
                }
            }
