        ...
        public override bool PreAI()
        {
            ...
            if (player.channel)
            {
                if (Projectile.ai[0] >= 1 && Projectile.soundDelay >= 0)
                {
                    for (int i = 0; i < 10; i++)
                    {
                        int dust = Dust.NewDust(player.position, player.width, player.height, DustID.PortalBolt);
            ...
            else
            {
                if (Projectile.localAI[1] <= 0)
                {
                    ...
                    SoundEngine.PlaySound(new SoundStyle("JoostMod/Sounds/Custom/MissileShoot"), Projectile.Center);
