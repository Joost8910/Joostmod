...
{
    public class DreamGreatSlash : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Dream Great Slash");
        ...
        public override bool PreAI()
        {
            ...
            {
                for (int i = 0; i < 12; i++)
                    Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.WitherLightning, 0, 0, 100, default, 1.3f + Main.rand.Next(5) / 10).noGravity = true;
            }
            Projectile.ai[0]++;
        ...
        {
            Player player = Main.player[Projectile.owner];
            if (Projectile.velocity.Y * player.gravDir > 0 && player.velocity.Y * player.gravDir > 0 && Math.Abs(Projectile.velocity.X) < 8)
            {
