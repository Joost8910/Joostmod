    ...
    public class SwingyHook : ModProjectile
    {
        ...
        public override void PostAI()
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                int grapCount = 0;
                int lastProj = -1;
                int lastProjTimeLeft = 100000;
                int maxGraps = 1;
                for (int i = 0; i < 1000; i++)
                {
                    if (Main.projectile[i].timeLeft < lastProjTimeLeft)
                    {
                        lastProj = i;
                        lastProjTimeLeft = Main.projectile[i].timeLeft;
                    }
                    grapCount++;
                }
                if (grapCount > maxGraps)
                {
                    Main.projectile[lastProj].Kill();
                }
            }
            */
        ...
        public override bool PreDrawExtras()
        {
            ...
            while (flag2)
            {
                ...
                else
                {
                    ...
                    Main.EntitySpriteDraw(TextureAssets.Chain16.Value, new Vector2(value.X - Main.screenPosition.X + TextureAssets.Chain16.Value.Width * 0.5f, value.Y - Main.screenPosition.Y + TextureAssets.Chain16.Value.Height * 0.5f), new Rectangle?(new Rectangle(0, 0, TextureAssets.Chain16.Value.Width, (int)num)), color2, rotation2, new Vector2(TextureAssets.Chain16.Value.Width * 0.5f, 0f), 1f, SpriteEffects.None, 0);
                }
            }
            return false;
        }
