	...
	public class Fireball : ModProjectile
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			for (i = 0; i < shootNum;i++ )
			{
				...
				Projectile.NewProjectile(Projectile.Center.X, Projectile.Center.Y, baseSpeed*(float)Math.Sin(offsetAngle), baseSpeed*(float)Math.Cos(offsetAngle), Mod.Find<ModProjectile>("Flamethrown").Type, Projectile.damage, 0, Projectile.owner);
			}
