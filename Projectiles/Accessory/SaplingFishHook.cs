        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[1] > 0f && Projectile.localAI[1] >= 0f)
            {
                ...
                if (!Projectile.lavaWet && !Projectile.honeyWet)
                {
                    for (int i = 0; i < 100; i++)
                    {
                        int num = Dust.NewDust(new Vector2(Projectile.position.X - 6f, Projectile.position.Y - 10f), Projectile.width + 12, 24, Dust.dustWater(), 0f, 0f, 0, default, 1f);
            ...
            if (Projectile.ai[0] >= 1f)
            {
                if (Projectile.ai[0] == 2f)
                {
                    ...
                    if (!Projectile.lavaWet && !Projectile.honeyWet)
                    {
                        for (int j = 0; j < 100; j++)
                        {
                            int num2 = Dust.NewDust(new Vector2(Projectile.position.X - 6f, Projectile.position.Y - 10f), Projectile.width + 12, 24, Dust.dustWater(), 0f, 0f, 0, default, 1f);
                ...
                float num7 = (float)Math.Sqrt((double)(xDif * xDif + yDif * yDif));
                if (num7 > 3000f)
                {
                    Projectile.Kill();
                }
                ...
                xDif *= num7;
                yDif *= num7;
                Projectile.velocity.X = (Projectile.velocity.X * (num4 - 1) + xDif) / num4;
                Projectile.velocity.Y = (Projectile.velocity.Y * (num4 - 1) + yDif) / num4;
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    if (rectangle.Intersects(value))
                    {
                        if (Projectile.ai[1] > 0f)
                        {
                            ...
                            if (num8 == 3196)
                            {
                                int num9 = fishingAttempt.fishingLevel;
                            ...
                            if (num8 == 3197)
                            {
                                int num11 = fishingAttempt.fishingLevel;
                            ...
                            Item item2 = player.GetItem(Projectile.owner, item, GetItemSettings.PickupItemFromWorld);
                            if (item2.stack > 0)
                            {
                                int number = Item.NewItem(Projectile.GetSource_FromAI(), (int)Projectile.position.X, (int)Projectile.position.Y, Projectile.width, Projectile.height, num8, 1, false, 0, true, false);
            ...
            if (Projectile.wet)
            {
                ...
                int num16 = (int)(Projectile.Center.X + (Projectile.width / 2 + 8) * Projectile.direction) / 16;
                int num17 = (int)(Projectile.Center.Y / 16f);
                float num18 = Projectile.position.Y / 16f;
                int num19 = (int)((Projectile.position.Y + Projectile.height) / 16f);
                /*
                ...
                */
                ...
                float num20 = Projectile.position.Y + Projectile.height;
                /*
                ...
                */
                ...
                {
                    num20 = num17 * 16;
                    num20 -= Main.tile[num16, num17 - 1].LiquidAmount / 16;
                }
                else if (Main.tile[num16, num17].LiquidAmount > 0)
                {
                    num20 = (num17 + 1) * 16;
                    num20 -= Main.tile[num16, num17].LiquidAmount / 16;
                }
                ...
                if (Projectile.velocity.Y >= -0.01 && Projectile.velocity.Y <= 0.01)
                {
                    flag = true;
                }
            }
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                int num21 = fishingAttempt.fishingLevel;
                if (num21 < 0 && num21 == -1)
                {
            ...
            if (flag)
            {
                ...
                {
                    int num22 = fishingAttempt.fishingLevel;
                    if (num22 == -9000)
                    {
                ...
                {
                    if (Projectile.velocity.Y == 0f || Projectile.honeyWet && Projectile.velocity.Y >= -0.01 && Projectile.velocity.Y <= 0.01)
                    {
                        Projectile.velocity.Y = Main.rand.Next(100, 500) * 0.015f;
                        Projectile.velocity.X = Main.rand.Next(-100, 101) * 0.015f;
        ...
        private void AutoFish()
        {
            ...
            if (player.whoAmI == Main.myPlayer && Projectile.ai[0] == 0f && Projectile.ai[1] < 0f && Projectile.localAI[1] != 0f)
            {
                ...
                if (flag4)
                {
                    ...
                    else if (Main.rand.NextBool(7) && !player.accFishingLine)
                    {
                        Projectile.ai[0] = 2f;
                    }
                    else
        ...
        public override bool PreDrawExtras()     //projectile draws the fishing line correctly
        {
            ...
            while (flag2)
            {
                ...
                else
                {
                    ...
                    Main.EntitySpriteDraw(TextureAssets.FishingLine.Value, new Vector2(value.X - Main.screenPosition.X + TextureAssets.FishingLine.Value.Width * 0.5f, value.Y - Main.screenPosition.Y + TextureAssets.FishingLine.Value.Height * 0.5f), new Microsoft.Xna.Framework.Rectangle?(new Rectangle(0, 0, TextureAssets.FishingLine.Value.Width, (int)num)), color2, rotation2, new Vector2(TextureAssets.FishingLine.Value.Width * 0.5f, 0f), 1f, SpriteEffects.None, 0);
                }
            }
            return false;
        }
