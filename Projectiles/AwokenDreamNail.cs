    ...
    public class AwokenDreamNail : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (channeling)
            {
                ...
                {
                    SoundEngine.PlaySound(SoundLoader.customSoundType, (int)Projectile.Center.X, (int)Projectile.Center.Y, Mod.GetSoundSlot(SoundType.Custom, "Sounds/Custom/hero_dream_nail_short_charge"));
                }
        ...
        public override void Kill(int timeLeft)
        {
            if(Projectile.ai[0] > 60)
            {
                ...
                if (player.controlUp || player.controlDown)
                {
                    ...
                    Projectile.NewProjectile(pos.X, pos.Y, player.direction, 0, Mod.Find<ModProjectile>("AwokenDreamSpinSlash").Type, Projectile.damage, Projectile.knockBack, Projectile.owner);
                }
                else if(Math.Abs(player.velocity.X) >= 6 && player.velocity.X * Projectile.velocity.X > 0 && Math.Abs(Projectile.velocity.Y) < 7)
                {
                    ...
                    Projectile.NewProjectile(pos.X, pos.Y, player.direction * 60, 0, Mod.Find<ModProjectile>("AwokenDreamDashSlash").Type, Projectile.damage * 9, Projectile.knockBack * 7, Projectile.owner);
                    player.velocity.X += 7 * player.direction;
                }
                else
                {
                    SoundEngine.PlaySound(SoundLoader.customSoundType, (int)Projectile.Center.X, (int)Projectile.Center.Y, Mod.GetSoundSlot(SoundType.Custom, "Sounds/Custom/hero_dream_nail_slash"));
                    Projectile.NewProjectile(pos.X, pos.Y, Projectile.velocity.X * 2, Projectile.velocity.Y * 2, Mod.Find<ModProjectile>("AwokenDreamGreatSlash").Type, Projectile.damage * 9, Projectile.knockBack * 7, Projectile.owner);
                }
            }
            else
            {
                ...
                Projectile.NewProjectile(pos.X, pos.Y, Projectile.velocity.X, Projectile.velocity.Y, Mod.Find<ModProjectile>("AwokenDreamNail2").Type, Projectile.damage, Projectile.knockBack, Projectile.owner);
                player.itemTime = player.itemAnimationMax;
                player.itemAnimation = player.itemAnimationMax;
                if (player.statLife >= player.statLifeMax2)
                {
                    Projectile.NewProjectile(pos.X, pos.Y, Projectile.velocity.X, Projectile.velocity.Y, Mod.Find<ModProjectile>("AwokenDreamBeam").Type, Projectile.damage / 2, 0, Projectile.owner);
                }
            }
        }
    }
