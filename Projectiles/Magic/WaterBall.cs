...
namespace JoostMod.Projectiles.Magic
{
    public class WaterBall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.channel && !player.noItems && !player.CCed && !player.dead && Projectile.ai[1] <= 0)
            {
                ...
                if (Projectile.ai[0] < 25 && Projectile.timeLeft == 1200)
                {
                    ...
                    if (Main.raining && Collision.CanHitLine(Projectile.position, Projectile.width, Projectile.height, new Vector2(Projectile.Center.X + (Main.screenPosition.Y - Projectile.Center.Y) / 14f * Main.windSpeedCurrent * 12f, Main.screenPosition.Y - 20), 1, 1) && Main.screenPosition.Y <= Main.worldSurface * 16.0)
                    {
            ...
            else
            {
                ...
                if (Projectile.velocity.Y < 5)
                {
                    ...
                    if (Main.tile[Projectile.Center.ToTileCoordinates().X, Projectile.Center.ToTileCoordinates().Y].LiquidAmount < 150)
        ...
        public override void Kill(int timeLeft)
        {
            ...
            tile.LiquidType = 0;
            tile.LiquidAmount += (byte)(int)Projectile.localAI[0];
            ...
            {
                Dust.NewDustDirect(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, DustID.DungeonWater, vel.X, vel.Y, 0, default, 1f + Projectile.ai[0] / 16).noGravity = true;
