        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner && Projectile.ai[1] <= 0)
            {
                ...
                else
                {
                    ...
                    Projectile.netUpdate = true;
                    SoundEngine.PlaySound(new("Terraria/Sounds/Custom/dd2_ghastly_glaive_pierce_0"), Projectile.Center); // 204
                }
            }
            if (Projectile.ai[1] <= 0)
            {
                Projectile.spriteDirection = Projectile.direction;
                Projectile.rotation = (float)Math.Atan2(Projectile.velocity.Y, Projectile.velocity.X) + 1.57f + 0.785f * Projectile.direction;
