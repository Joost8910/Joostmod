...
{
	public abstract class GroundShooter : Minion
	{
		...
		public override void Behavior()
		{
			...
			Vector2 playerPos = player.Center + new Vector2(((10 + num * 40) * -player.direction), 0);
			if (projectile.ai[0] == 1f && Vector2.Distance(playerPos, projectile.Center) > 200)
			{
				projectile.tileCollide = false;
				projectile.velocity = projectile.DirectionTo(playerPos) * 10;
			...
			else
			{
				...
				if (!Collision.CanHitLine(projectile.Center, 1, 1, player.Center, 1, 1) && direction.Length() >= 200f)
				{
					projectile.ai[0] = 1f;
				}
				float speed = 6f;
