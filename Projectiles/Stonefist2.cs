...
namespace JoostMod.Projectiles
{
    public class Stonefist2 : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            {
                SoundEngine.PlaySound(SoundID.Trackable.WithPitchOffset(-0.3f), Projectile.position);
                ...
                if (target.active && target.statLife > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (Projectile.ai[0] > 2)
                        {
                            ...
                            Projectile.NewProjectile(target.Center, player.velocity + aim * Projectile.knockBack, Mod.Find<ModProjectile>("GrabThrow").Type, Projectile.damage * 2, Projectile.knockBack, Projectile.owner, -1, target.whoAmI);
                            if (player.immuneTime < 10)
                            {
                                player.immune = true;
                                player.immuneNoBlink = false;
