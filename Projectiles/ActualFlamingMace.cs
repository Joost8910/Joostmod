	...
	public class ActualFlamingMace : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            else if (Projectile.soundDelay != -10)
            {
                ...
                SoundEngine.PlaySound(SoundID.Trackable.WithVolumeScale(0.9f).WithPitchOffset(0.1f), Projectile.Center);
            }
            if (Projectile.timeLeft <= 120)
            {
                ...
                if (Projectile.timeLeft == 68 && Projectile.ai[0] < 100)
                {
                    ...
                    {
                        SoundEngine.PlaySound(SoundID.Trackable.WithVolumeScale(0.8f).WithPitchOffset(0.1f), Projectile.Center);
                    }
                }
                if (Projectile.ai[1] > 180)
                {
            ...
            if (Projectile.localAI[1] == 0)
            {
                ...
                if (Main.tile[tilePos.X, tilePos.Y].LiquidAmount > 200 && Main.tile[tilePos.X, tilePos.Y].LiquidType != 2)
                {
                    Projectile.localAI[1] = 1;
                    SoundEngine.PlaySound(SoundID.Item13.WithVolumeScale(0.8f).WithPitchOffset(0.2f), Projectile.position);
                }
            }
        }
        ...
        public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            ...
            if (target.knockBackResist > 0)
            {
                if (Projectile.ai[0] >= 100 && Projectile.ai[1] > 120)
                {
                    ...
                    {
                        Projectile.NewProjectile(target.Center, target.velocity, Mod.Find<ModProjectile>("GrabThrow").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, target.whoAmI);
                    }
        ...
        public override void ModifyHitPvp(Player target, ref int damage, ref bool crit)
        {
            ...
            if (player.velocity.Y * player.gravDir > 3 && Projectile.ai[1] > 120)
            {
                if (target.velocity.Y < 0 && Projectile.localAI[0] > maxCharge * 0.9f)
                {
                    Projectile.NewProjectile(target.Center, target.velocity, Mod.Find<ModProjectile>("GrabThrow").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, -1, target.whoAmI);
                }
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            if (Projectile.localAI[1] == 0)
            {
                target.AddBuff(BuffID.OnFire, (int)(360 * Projectile.localAI[0]) + 144);
            }
        }
