		...
		public override void Kill(int timeLeft)
		{
			...
			if (tileSafely.HasTile)
			{
				...
				if (!tileSafely2.HasTile || !Main.tileSolid[(int)tileSafely2.TileType] || Main.tileSolidTop[(int)tileSafely2.TileType])
				{
					...
					Projectile.NewProjectile(Projectile.Center.X, Projectile.Center.Y + 32 + (int)(-56 * Projectile.ai[1]), 0, 0, Mod.Find<ModProjectile>("GrogWave2").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, Projectile.ai[0], Projectile.ai[1]);					
				}
			}
		}
	}
