    ...
    public class CoiledNail : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (channeling)
            {
                ...
                if (Projectile.ai[0] >= 20 && Projectile.ai[0] < 100)
                {
                ...
                if (Projectile.ai[0] > 100)
                {
                    if ((Projectile.ai[0] + 20) % 80 == 40)
                    {
                        SoundEngine.PlaySound(SoundLoader.customSoundType, (int)Projectile.Center.X, (int)Projectile.Center.Y, Mod.GetSoundSlot(SoundType.Custom, "Sounds/Custom/hero_nail_art_charge_loop_1"));
                    }
                    if ((Projectile.ai[0] + 20) % 80 == 0)
                    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            else
            {
                ...
                Projectile.NewProjectile(pos.X, pos.Y, Projectile.velocity.X, Projectile.velocity.Y, Mod.Find<ModProjectile>("CoiledNail2").Type, Projectile.damage, Projectile.knockBack, Projectile.owner);
