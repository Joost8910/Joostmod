    ...
    public class DreamNail : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            if(projectile.ai[0] > 75)
            {
                ...
                if (Math.Abs(player.velocity.X) > 7 && player.velocity.X * projectile.velocity.X > 0 && Math.Abs(projectile.velocity.Y) < 6)
                {
            ...
            else
            {
                ...
                Projectile.NewProjectile(pos.X, pos.Y, projectile.velocity.X, projectile.velocity.Y, mod.ProjectileType("DreamNail2"), projectile.damage, projectile.knockBack, projectile.owner);
                player.itemTime = player.itemAnimationMax;
                player.itemAnimation = player.itemAnimationMax;
                if (player.statLife >= player.statLifeMax2)
                {
                    Projectile.NewProjectile(pos.X, pos.Y, projectile.velocity.X, projectile.velocity.Y, mod.ProjectileType("DreamBeam"), projectile.damage / 2, 0, projectile.owner);
                }
            }
        }
    }
