...
namespace JoostMod
{
    public class JoostMod : Mod
    {
        ...
        public override void PostSetupContent()
        {
            ...
            if (ModLoader.TryGetMod("BossChecklist", out Mod bossChecklist))
            {
                ...
                */
                ...
                bossChecklist.Call("AddBoss", 15.1f, ModContent.NPCType<SAX>(), this, "SA-X", (Func<bool>)(() => JoostWorld.downedSAX), ModContent.ItemType<InfectedArmCannon>(), new List<int>() { ModContent.ItemType<Items.Armor.SAXMask>(), ModContent.ItemType<Items.Placeable.SAXMusicBox>(), ModContent.ItemType<Items.Placeable.SAXTrophy>() }, new List<int>() { ModContent.ItemType<XBag>(), ModContent.ItemType<XShield>(), ModContent.ItemType<IceCoreX>() }, "Use an [i:" + ModContent.ItemType<InfectedArmCannon>() + "] anywhere", "The SA-X disappears into the unknown", "JoostMod/NPCs/Bosses/SAXBossLog");
        ...
        public void InitializeHunts()
        {
            ...
            new HuntInfo(ModContent.NPCType<IdleCactusWorm>(), 5.7f, "Ya know those cactus worms? They're the most delicious kind of cactus! Anyways, there seems to be some huge one in the underground desert. You ought to use that cannon to clear out a large enough space to fight it, it's gonna be mighty nasty when you wake it up!", "Hang on here, so you killed the big cacto worm, and then an even BIGGER one showed up after? Jegus, you should be glad you weren't killed too hard! Anyways, it seems there's less cactus worms now that you killed their leader. I think the cactus people will finally be able to move back into the desert now! Ya better make a house for them.", ModContent.ItemType<Items.Quest.GrandCactusWorm>(), () => NPC.downedBoss3, () => JoostWorld.downedCactusWorm, () => true, CactoWormReward),
            new HuntInfo(ModContent.NPCType<ImpLord>(), 5.8f, "Go to hell! Literally! Dig down until ya find yerself surrounded by lava and demons! Among those demons are fire imps: little teleportin' buggers that shoot fire! They have a leader with wings that's especially dangerous. Watch out for his giant fire blast attack! Ya can use a sword to reflect it back at him but if you fail to it hurts a lot!", "Good job taking down the Imp Lord! Here, ya can use the tail as a whip or somethin'", ModContent.ItemType<Items.Quest.ImpLord>(), () => JoostWorld.downedCactusWorm, () => JoostWorld.downedImpLord, () => true, ImpReward),
